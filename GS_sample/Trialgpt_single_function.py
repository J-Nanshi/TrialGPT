...
client = OpenAI(
    ...
    api_key=config['OpenAI.Science-vNext-Internal']['api_key'],
)
...
def Clinical_trial_matching(CT_DB_name, TopK_retrieval, Patient_query):
    ...
    qid2nctids = {} #
    recalls = []
    query = outputs_patient_keyword["Patient_details"]["raw"]
    qid = "Patient_details"
    ...
    # a list of nctid lists for the bm25 retriever
        ...
        for ind_list in inds:
            top_nctids = [medcpt_nctids[ind] for ind in ind_list] # here they are retrieving the NCT_IDS for each index present in ind_list map corresponding to index of medcpt_ncts
