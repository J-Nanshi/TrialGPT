...
agg_results_path = "../../GS_sample/dataset/GS_data/token_counts/Results/P_NCT02448420_LLM_aggregation_results.json"
logging.basicConfig(filename='../../GS_sample/dataset/GS_data/token_counts/Results/P_NCT02448420_final_score_list.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
# loading the results
...
for patient_id, label2trial2results in matching_results.items():
	...
	logging.info(f"Clinical trial ranking:")
	for trial, score in sorted_trial2score:
		print(trial, score)
		logging.info(f"{trial} : {score}")
